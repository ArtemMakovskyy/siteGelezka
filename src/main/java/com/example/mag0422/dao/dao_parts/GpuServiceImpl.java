...
import com.example.mag0422.dao.dao_parts.repository.GpuDBRepository;
...
@Service
public class GpuServiceImpl implements PartsService<GpuDB> {

    private final GpuDBRepository repository;
    private GpuDB db;

    @Autowired
    public GpuServiceImpl(GpuDBRepository repository) {
        this.repository = repository;
    }

    @Override
    public List<GpuDB> getAll() {
        return repository.findAll();
    }

    @Override
    public void save(GpuDB db) {
        repository.save(db);
    }

    @Override
    public GpuDB findById(long id) {
        Optional<GpuDB> optional = repository.findById(id);
        GpuDB db = null;
    ...
    @Override
    public Page<GpuDB> findPaginatedFourFilds
    ...
    @Override
    public Page<GpuDB> findPaginatedByPageNPageAize
    ...
    public void rememberObject(Long id) {
        if (this.db == null) this.db = new GpuDB();
        this.db = findById(id);
    }

    @Override
    public GpuDB getRememberedObject() {
        if (this.db == null) {
    ...
    @Override
    public List<GpuDB> findAllTextFields(String serchingText) {
        if (serchingText == null) {
            System.out.println("нет данных для поиска");
            serchingText = " ";
        }
        String textInLowerCase = serchingText.toLowerCase();
        List<GpuDB> findingList = new ArrayList<>();

        for (GpuDB obj : repository.findAll())
